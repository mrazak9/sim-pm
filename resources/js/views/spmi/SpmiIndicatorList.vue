    ...
    <div class="rounded-lg border border-gray-200 bg-white shadow dark:border-gray-700 dark:bg-gray-800">
      ...
      <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
        <div class="flex items-center justify-between">
          ...
          <button
            @click="$router.push('/spmi/indicators/create')"
      ...
      <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
        <div class="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <div>
            <select
              v-model="filters.spmi_standard_id"
          ...
          <div>
            <select
              ...
              @change="fetchIndicators()"
            >
              <option value="">Semua Tipe Pengukuran</option>
        ...
        <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
          <thead class="bg-gray-50 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
            <tr>
              ...
              <th scope="col" class="px-6 py-3">Tipe Pengukuran</th>
          ...
          <tbody>
            ...
            <tr v-else v-for="indicator in indicators" :key="indicator.id" class="border-b border-gray-200 bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
              <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                {{ indicator.code }}
              </td>
              <td class="px-6 py-4">
                <p class="font-medium text-gray-900 dark:text-white">{{ indicator.name }}</p>
                <p v-if="indicator.description" class="mt-1 text-xs text-gray-500 dark:text-gray-400 line-clamp-2">
                  {{ indicator.description }}
                </p>
              </td>
              <td class="px-6 py-4 text-gray-900 dark:text-white">
                {{ indicator.spmi_standard?.name || '-' }}
              </td>
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4">
                <span :class="getStatusClass(indicator.is_active)">
                  {{ indicator.is_active ? 'Aktif' : 'Tidak Aktif' }}
                </span>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center gap-2">
                  <button
                    v-if="!indicator.is_active"
                    @click="activateIndicator(indicator)"
                  ...
                  <button
                    v-if="indicator.is_active"
                    @click="deactivateIndicator(indicator)"
                    class="text-yellow-600 hover:text-yellow-700 dark:text-yellow-400"
                    title="Nonaktifkan"
                  >
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l-2-2m0 0l-2-2m2 2l2-2m-2 2l-2 2m8-2l-2 2m0 0l-2-2m2 2l2-2m-2 2l-2 2" />
                    </svg>
                  </button>
                  <button
                    @click="$router.push(`/spmi/indicators/${indicator.id}/edit`)"
      ...
      <div v-if="pagination.total > 0" class="flex items-center justify-between border-t border-gray-200 px-6 py-4 dark:border-gray-700">
        <div class="text-sm text-gray-700 dark:text-gray-400">
          ...
          <span class="font-medium text-gray-900 dark:text-white">{{ pagination.total }}</span> indikator
        </div>
...
const fetchIndicators = async (page = 1) => {
  try {
    ...
    const response = await getSPMIIndicators(params);
...
const fetchStatistics = async () => {
  try {
    const response = await getSPMIIndicatorStatistics();
    if (response.success) {
...
const deactivateIndicator = async (indicator) => {
  if (confirm(`Apakah Anda yakin ingin menonaktifkan indikator "${indicator.name}"?`)) {
    try {
