    ...
    <div class="rounded-lg border border-gray-200 bg-white shadow dark:border-gray-700 dark:bg-gray-800">
      ...
      <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-700">
        <div class="flex items-center justify-between">
          ...
          <button
            @click="$router.push('/spmi/standards/create')"
      ...
      <div v-if="statistics" class="grid grid-cols-1 gap-4 border-b border-gray-200 px-6 py-4 dark:border-gray-700 md:grid-cols-5">
        ...
        <div class="rounded-lg bg-blue-50 p-4 dark:bg-blue-900/20">
          <p class="text-sm text-blue-600 dark:text-blue-400">Draft</p>
          <p class="text-2xl font-bold text-blue-700 dark:text-blue-300">{{ statistics.draft }}</p>
        </div>
        <div class="rounded-lg bg-green-50 p-4 dark:bg-green-900/20">
          <p class="text-sm text-green-600 dark:text-green-400">Aktif</p>
          <p class="text-2xl font-bold text-green-700 dark:text-green-300">{{ statistics.active }}</p>
        </div>
        <div class="rounded-lg bg-yellow-50 p-4 dark:bg-yellow-900/20">
          <p class="text-sm text-yellow-600 dark:text-yellow-400">Revisi</p>
          <p class="text-2xl font-bold text-yellow-700 dark:text-yellow-300">{{ statistics.revision }}</p>
        </div>
        <div class="rounded-lg bg-red-50 p-4 dark:bg-red-900/20">
          <p class="text-sm text-red-600 dark:text-red-400">Arsip</p>
          <p class="text-2xl font-bold text-red-700 dark:text-red-300">{{ statistics.archived }}</p>
        </div>
      </div>
        ...
        <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
          <thead class="bg-gray-50 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
            <tr>
              ...
              <th scope="col" class="px-6 py-3">Nama Standar</th>
          ...
          <tbody>
            ...
            <tr v-else-if="standards.length === 0">
              <td colspan="8" class="px-6 py-8 text-center text-gray-900 dark:text-white">
                Tidak ada standar SPMI
              </td>
            </tr>
            <tr v-else v-for="standard in standards" :key="standard.id" class="border-b border-gray-200 bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
              <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                {{ standard.code }}
              </td>
              <td class="px-6 py-4">
                <p class="font-medium text-gray-900 dark:text-white">{{ standard.name }}</p>
                <p v-if="standard.description" class="mt-1 text-xs text-gray-500 dark:text-gray-400 line-clamp-2">
              ...
              <td class="px-6 py-4 text-gray-900 dark:text-white">
                {{ formatDate(standard.effective_date) }}
              </td>
              <td class="px-6 py-4 text-gray-900 dark:text-white">
                {{ formatDate(standard.review_date) }}
              </td>
              <td class="px-6 py-4">
              ...
              <td class="px-6 py-4">
                <div class="flex items-center gap-2">
                  <button
                    ...
                    @click="approveStandard(standard)"
                  ...
                  <button
                    v-if="standard.status === 'active'"
                    @click="archiveStandard(standard)"
                    class="text-red-600 hover:text-red-700 dark:text-red-400"
                    title="Arsipkan"
                  >
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9-4v4m4-4v4" />
                    </svg>
                  </button>
                  <button
                    @click="$router.push(`/spmi/standards/${standard.id}/edit`)"
      ...
      <div v-if="pagination.total > 0" class="flex items-center justify-between border-t border-gray-200 px-6 py-4 dark:border-gray-700">
        <div class="text-sm text-gray-700 dark:text-gray-400">
          ...
          <span class="font-medium text-gray-900 dark:text-white">{{ pagination.total }}</span> standar SPMI
        </div>
...
const { getSPMIStandards, getSPMIStandardStatistics, deleteSPMIStandard, approveSPMIStandard, reviseSPMIStandard, archiveSPMIStandard, loading } = useSPMIApi();
...
const fetchStandards = async (page = 1) => {
  try {
    ...
    const response = await getSPMIStandards(params);
...
const fetchStatistics = async () => {
  try {
    const response = await getSPMIStandardStatistics();
    if (response.success) {
...
const approveStandard = async (standard) => {
  if (confirm(`Apakah Anda yakin ingin menyetujui standar "${standard.name}"?`)) {
    try {
      const response = await approveSPMIStandard(standard.id);
      if (response.success) {
        alert('Standar berhasil disetujui');
        fetchStandards(pagination.value.current_page);
        fetchStatistics();
      }
    } catch (error) {
      alert('Gagal menyetujui standar: ' + (error.response?.data?.message || error.message));
    }
  }
};

const reviseStandard = async (standard) => {
  if (confirm(`Apakah Anda yakin ingin merevisi standar "${standard.name}"?`)) {
    try {
      const response = await reviseSPMIStandard(standard.id);
      if (response.success) {
        alert('Standar diatur untuk revisi');
        fetchStandards(pagination.value.current_page);
        fetchStatistics();
      }
    } catch (error) {
      alert('Gagal merevisi standar: ' + (error.response?.data?.message || error.message));
    }
  }
};

const archiveStandard = async (standard) => {
  if (confirm(`Apakah Anda yakin ingin mengarsipkan standar "${standard.name}"?`)) {
    try {
      const response = await archiveSPMIStandard(standard.id);
      if (response.success) {
        alert('Standar berhasil diarsipkan');
        fetchStandards(pagination.value.current_page);
        fetchStatistics();
      }
    } catch (error) {
      alert('Gagal mengarsipkan standar: ' + (error.response?.data?.message || error.message));
    }
  }
};

const confirmDelete = async (standard) => {
  if (confirm(`Apakah Anda yakin ingin menghapus standar "${standard.name}"?`)) {
    try {
      const response = await deleteSPMIStandard(standard.id);
      if (response.success) {
      ...
      alert('Gagal menghapus standar: ' + (error.response?.data?.message || error.message));
    }
  }
};
