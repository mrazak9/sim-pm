      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-sm text-gray-500 dark:text-gray-400">
          ...
          <tbody>
            ...
            <tr v-else-if="programStudis.length === 0">
              <td colspan="7" class="px-6 py-8 text-center text-gray-900 dark:text-white">
                Tidak ada data program studi
              </td>
            </tr>
            <tr v-else v-for="item in programStudis" :key="item.id" class="border-b border-gray-200 bg-white hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700">
              <td class="px-6 py-4 font-medium text-gray-900 dark:text-white">
                {{ item.kode_prodi }}
              </td>
              <td class="px-6 py-4">
                <p class="font-medium text-gray-900 dark:text-white">{{ item.nama_prodi }}</p>
              ...
              <td class="px-6 py-4">
                <span :class="getAkreditasiClass(item.akreditasi)">
                  {{ item.akreditasi || 'Belum' }}
                </span>
              </td>
              <td class="px-6 py-4">
                <span :class="[
...
const { loading, getProgramStudis, deleteProgramStudi } = useMasterDataApi();

const programStudis = ref([]);
const filters = ref({
  search: '',
  is_active: '',
  jenjang: '',
...
const fetchProgramStudis = async (page = 1) => {
  try {
    const params = {
      page,
...
const changePage = (page) => {
  if (page >= 1 && page <= pagination.value.last_page) {
    fetchProgramStudis(page);
  }
};

const getAkreditasiClass = (akreditasi) => {
  if (!akreditasi || akreditasi === 'Belum Terakreditasi') {
    return 'inline-flex rounded-full bg-gray-100 px-3 py-1 text-xs font-medium text-gray-800 dark:bg-gray-900 dark:text-gray-300';
  }
  if (akreditasi === 'Unggul' || akreditasi === 'A') {
    return 'inline-flex rounded-full bg-green-100 px-3 py-1 text-xs font-medium text-green-800 dark:bg-green-900 dark:text-green-300';
  }
  if (akreditasi === 'Baik Sekali' || akreditasi === 'B') {
    return 'inline-flex rounded-full bg-blue-100 px-3 py-1 text-xs font-medium text-blue-800 dark:bg-blue-900 dark:text-blue-300';
  }
  return 'inline-flex rounded-full bg-yellow-100 px-3 py-1 text-xs font-medium text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300';
};

const openCreateForm = () => {
  router.push({ name: 'program-studi-create' });
};

const editItem = (item) => {
  router.push({ name: 'program-studi-edit', params: { id: item.id } });
};

const confirmDelete = async (item) => {
  if (confirm(`Apakah Anda yakin ingin menghapus program studi "${item.nama_prodi}"?`)) {
    try {
      await deleteProgramStudi(item.id);
      fetchProgramStudis(pagination.value.current_page);
    } catch (error) {
      console.error('Failed to delete program studi:', error);
