  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800">
      ...
      <div class="mb-6 rounded-lg bg-gray-50 p-4 dark:bg-gray-700">
        <p class="mb-1 text-sm text-gray-500 dark:text-gray-400">Dokumen saat ini:</p>
        <p class="font-medium text-gray-900 dark:text-white">{{ document.title }}</p>
        <p class="text-sm text-gray-600 dark:text-gray-300">
          Versi {{ document.current_version }} • {{ document.file_name }}
        </p>
      </div>
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <div class="mb-4">
          ...
          ></textarea>
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            Catatan ini akan membantu pengguna lain memahami perubahan yang dilakukan
          </p>
        </div>

        <!-- Version Preview -->
        <div v-if="selectedFile" class="mb-4 rounded-lg bg-blue-50 p-3 dark:bg-blue-900/20">
          <p class="text-sm text-blue-800 dark:text-blue-300">
            <strong>Versi baru:</strong> v{{ document.current_version + 1 }}
          </p>
        </div>
        ...
        <div v-if="uploading" class="mb-4">
          <div class="flex items-center justify-between text-sm text-gray-700 dark:text-gray-300">
            <span>Mengupload versi baru...</span>
        ...
        <div class="flex justify-end space-x-3">
          ...
          >
            {{ uploading ? 'Mengupload...' : 'Upload Versi Baru' }}
          </button>
        </div>
      </form>
    </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (changeNotes.value) {
      formData.append('change_notes', changeNotes.value);
    }

    const response = await axios.post(
      `/api/documents/${props.document.id}/upload-version`,
      formData,
      {
        headers: {
    ...
    errorMessage.value = error.response?.data?.message || 'Gagal mengupload versi baru';
